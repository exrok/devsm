...
fn npm_additions(root: &Path, _kept: &[PathBuf]) -> Vec<PathBuf> {
    first_existing(root, &["package-lock.json", "pnpm-lock.yaml", "yarn.lock"]).into_iter().collect()
}

fn python_additions(root: &Path, _kept: &[PathBuf]) -> Vec<PathBuf> {
    first_existing(root, &["poetry.lock", "uv.lock", "requirements.txt"]).into_iter().collect()
}
...
pub const RULES: &[FrameworkRule] = &[
    ...
    FrameworkRule { name: "python", triggers: &["pyproject.toml", "setup.py"], additions: python_additions },
];
