...
impl DecodingState {
    ...
    pub fn decode<'a>(&mut self, bytes: &'a [u8]) -> DecodeResult<'a> {
        ...
        if remaining.len() < HEAD_SIZE {
            return DecodeResult::MissingData {
                additional: HEAD_SIZE - remaining.len(),
            };
        }
        ...
        if remaining.len() < total_size {
            return DecodeResult::MissingData {
                additional: total_size - remaining.len(),
            };
        }
...
impl Encoder {
    ...
    pub fn encode_response<T: jsony::ToBinary>(
        &mut self,
        kind: RpcMessageKind,
        correlation: u16,
        payload: &T,
    ...
    fn encode_with_correlation<T: jsony::ToBinary>(
        &mut self,
        kind: RpcMessageKind,
        correlation: u16,
        payload: &T,
...
mod tests {
    ...
    fn encoder_decoder_round_trip() {
        let mut encoder = Encoder::new();
        let payload = ResizeNotification {
            width: 120,
            height: 40,
        };
    ...
    fn decoder_multiple_messages() {
        let mut encoder = Encoder::new();
        encoder.encode_push(RpcMessageKind::JobStatus, &JobStatusEvent { job_index: 1, status: JobStatusKind::Running });
        encoder.encode_push(RpcMessageKind::JobStatus, &JobStatusEvent { job_index: 2, status: JobStatusKind::Waiting });
        let data = encoder.output().to_vec();
